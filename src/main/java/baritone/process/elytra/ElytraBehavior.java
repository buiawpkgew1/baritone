            ...
            return this.path0(from, ElytraBehavior.this.destination, UnaryOperator.identity())
                    .thenRun(() -> {
                        ...
                        if (this.completePath) {
                            logVerbose(String.format("计算的路径 (%.1f 个方块在 %.4f 秒内)", distance, (System.nanoTime() - start) / 1e9d));
                        } else {
                            logVerbose(String.format("计算的段落 (接下来 %.1f 个方块在 %.4f 秒内)", distance, (System.nanoTime() - start) / 1e9d));
                        }
                    })
